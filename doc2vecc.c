...
char train_file[MAX_STRING], test_file[MAX_STRING], output_file[MAX_STRING], wordembedding_file[MAX_STRING];
...
real alpha = 0.025, starting_alpha, sample = 1e-3, rp_sample=0.1;
real *syn0, *syn1, *syn1neg, *expTable, *syn0u;
clock_t start;

// the part of code on hs is not working
int hs = 0, negative = 5;
...
void *TrainModelThread(void *id) {
  ...
  real f, g, w, maxw, totalw, sen_w[MAX_SENTENCE_SAMPLE], syn0tmp;
  clock_t now;
  real *neu1 = (real *)calloc(layer1_size, sizeof(real));
  real *neu1e = (real *)calloc(layer1_size, sizeof(real));
  int *neighbor = (int *)calloc(MAX_SENTENCE_LENGTH, sizeof(int));
...
void TrainModel() {
  ...
  printf("finish embedding training\n");
  fflush(stdout);
	
  // Save the word vectors
  fo_w = fopen(wordembedding_file, "wb"); 
  fprintf(fo_w, "%lld %lld\n", vocab_size, layer1_size);
  for (a = 0; a < vocab_size; a++) {
	fprintf(fo_w, "%s ", vocab[a].word);
      	if (binary) for (b = 0; b < layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo_w);
  ...
  real f, g, w, maxw, totalw, sen_w[MAX_SENTENCE_LENGTH + 1];
  real *neu1 = (real *)calloc(layer1_size, sizeof(real));
...
int main(int argc, char **argv) {
  int i;
  if (argc == 1) {
    printf("DOCUMENT VECTOR estimation toolkit\n\n");
    printf("Options:\n");
    ...
    printf("\t-sentence-sample <float>\n");
    printf("\t\tThe rate to sample words out of a document for documenet representation\n");
    printf("\nExamples:\n");
    printf("./doc2vecc -train data.txt -output docvec.txt -word wordvec.txt -sentence-sample 0.1 -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-word", argc, argv)) > 0) strcpy(wordembedding_file, argv[i + 1]);
